...
/**
 * POST /api/admin/upload — admin-only file upload to ArvanCloud Object Storage.
 *
...
const UPLOAD_FOLDERS = {
  ...
  favicon: "uploads/branding/favicon",
  hero: "uploads/hero",
...
// Favicons may additionally be .ico files.
...
const MSG = {
  ...
  notConfigured:
    "سرویس آپلود پیکربندی نشده است. لطفاً تنظیمات فضای ذخیره‌سازی را بررسی کنید.",
...
export async function POST(request: Request) {
  ...
  if (isDocument) {
    ...
    const allowed =
      IMAGE_MIME.has(mime) ||
      (type === "favicon" && FAVICON_EXTRA_MIME.has(mime));
    if (!allowed) {
      return json({ ok: false, error: MSG.badMime }, 415);
    }
  ...
  // 4) Build a safe key + upload.
  const key = buildObjectKey(UPLOAD_FOLDERS[type], file.name || "file");
  ...
  try {
    ...
    const friendly = message.startsWith("Storage is not configured")
      ? MSG.notConfigured
      : MSG.failed;
